...
class StackOverflow extends Serializable {
  ...
  def scoredPostings(grouped: RDD[(Int, Iterable[(Posting, Posting)])]): RDD[(Posting, Int)] = {
    ...
    //grouped.map(element => (element._2.head._1, answerHighScore(element._2.map(_._2).toArray)))
  ...
  @tailrec final def kmeans(means: Array[(Int, Int)], vectors: RDD[(Int, Int)], iter: Int = 1, debug: Boolean = false): Array[(Int, Int)] = {
    ...
    val closest_mean = vectors.map(x => (findClosest(x, means), x)) //每一个点找出最近的聚类中心
    val closest_mean_grouped = closest_mean.groupByKey() //将一个聚类中心下的所有点合起来
    closest_mean_grouped.mapValues(x => averageVectors(x)).collect().foreach(pair => newMeans.update(pair._1, pair._2))
  ...
  def clusterResults(means: Array[(Int, Int)], vectors: RDD[(Int, Int)]): Array[(String, Double, Int, Int)] = {
    ...
    val median = closestGrouped.mapValues { vs =>
      val langIndex: Int = vs.groupBy(_._1).maxBy(_._2.size)._1
      val langLabel: String   = langs(langIndex / langSpread) // most common language in the cluster
      val clusterSize: Int    = vs.size
      val langPercent: Double = vs.count(_._1 == langIndex) * 100d / clusterSize // percent of the questions in the most common language
  ...
  def calMedian(vectors: Iterable[(Int, Int)]): Int = {
    val list = vectors.map(_._2).toList.sortWith(_ < _)
