...
object WikipediaRanking {
   ...
   */
  def occurrencesOfLang(lang: String, rdd: RDD[WikipediaArticle]): Int = rdd.aggregate(0)((num, article) => if(article.mentionsLanguage(lang)) num+1 else num, _+_)
  ...
  def makeIndex(langs: List[String], rdd: RDD[WikipediaArticle]): RDD[(String, Iterable[WikipediaArticle])] = {
    val list = rdd.flatMap(article => {
      val LangMentioned = langs.filter(lang => article.mentionsLanguage(lang))
