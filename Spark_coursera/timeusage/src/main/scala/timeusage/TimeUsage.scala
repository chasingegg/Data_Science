...
object TimeUsage {
  ...
  def dfSchema(columnNames: List[String]): StructType = {
    ...
    val tailField = columnNames.tail.map(p => StructField(p, DoubleType, nullable = false))
    StructType(firstField::tailField)
  }
  ...
  def row(line: List[String]): Row = {
    val l = line.head::line.tail.map(_.toDouble)
    Row.fromSeq(l)
  }
  ...
  /** @return The initial data frame columns partitioned in three groups: primary needs (sleeping, eating, etc.),
    ...
    */
  def classify(columnName: List[String], primary: List[Column], working: List[Column], other: List[Column]): (List[Column], List[Column], List[Column]) = {
    columnName match {
      ...
      case x:: xs =>
        if (x.startsWith("t01") || x.startsWith("t03") || x.startsWith("t11") || x.startsWith("t1801") || x.startsWith("t1803"))
          classify(xs,  col(x)::primary, working, other)
        else if (x.startsWith("t05") || x.startsWith("t1805"))
          classify(xs, primary, col(x)::working, other)
          ...
          classify(xs, primary, working,  col(x)::other)
        else
          classify(xs, primary, working, other)
    }
  }
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    ...
    classify(columnNames, primary, working, other)
  }
  ...
  def timeUsageGroupedTyped(summed: Dataset[TimeUsageRow]): Dataset[TimeUsageRow] = {
    ...
    summed groupByKey (
        row => (row.working, row.sex, row.age)
        ) agg (
